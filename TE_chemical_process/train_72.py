...
parser.add_argument("--ts", default=72, type=int, help="网络处理数据的时间步长")
...
X_train, y_train, mask_train = dataLoader.window_process(trainRoot, cfg.dl, 20, cfg.ts, cfg.lb)
...
def test(epoch):
    ...
    if test_acc > best_test_acc:
        ...
        torch.save(state, os.path.join('./checkpoints/res3_72', cfg.model + '-' + str(epoch) + '.t7'))
        best_test_acc = test_acc
        best_test_acc_epoch = epoch
        print('更新完成')
