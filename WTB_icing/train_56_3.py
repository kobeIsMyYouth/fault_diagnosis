...
parser.add_argument("--ts", default=56, type=int, help="网络处理数据的时间步长")
...
parser.add_argument("--rr", default="./data/r/r_0.03.txt", type=str, help="注意力矩阵文件路径")
parser.add_argument("--ep", default=100, type=int, help="训练的总轮数")
parser.add_argument('--re', default=True, help='是否从checkpoint处开始训练')
parser.add_argument("--ckpt", default="./checkpoints/res_56_3/CNN-LSTM-9.t7", help="checkpoint的地址")
...
def test(epoch):
    ...
    if test_acc > best_test_acc:
        ...
        torch.save(state, os.path.join('./checkpoints/res_56_3', cfg.model + '-' + str(epoch) + '.t7'))
        best_test_acc = test_acc
        best_test_acc_epoch = epoch
        print('更新完成')
